...
class ComprehensiveAadhaarPipeline:
    def __init__(self, model1_path, model2_path, output_base_dir="pipeline_output", confidence_threshold=0.10, other_lang_code='hin+tel+ben'):
        ...
        logger.info("🔁 Checking Redis cache for YOLO models...")
        model1_bytes = get_file_from_cache("model:best.pt")
        model2_bytes = get_file_from_cache("model:best_updated.pt")

        if model1_bytes:
            logger.info("✅ Model1 loaded from Redis")
            # Handle BytesIO or bytes
            if hasattr(model1_bytes, 'read'):
                model1_bytes.seek(0)
                model1_bytes_data = model1_bytes.read()
            else:
                model1_bytes_data = model1_bytes
            with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as f1:
                f1.write(model1_bytes_data)
                model1_temp_path = f1.name
            self.model1 = YOLO(model1_temp_path)
        else:
            logger.critical("❌ Model1 not found in Redis. Aborting startup.")
            raise FileNotFoundError("Model1 not found in Redis cache.")

        if model2_bytes:
            logger.info("✅ Model2 loaded from Redis")
            # Handle BytesIO or bytes
            if hasattr(model2_bytes, 'read'):
                model2_bytes.seek(0)
                model2_bytes_data = model2_bytes.read()
            else:
                model2_bytes_data = model2_bytes
            with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as f2:
                f2.write(model2_bytes_data)
                model2_temp_path = f2.name
            self.model2 = YOLO(model2_temp_path)
        else:
            logger.critical("❌ Model2 not found in Redis. Aborting startup.")
            raise FileNotFoundError("Model2 not found in Redis cache.")
        ...
        logger.info("YOLOv8 models loaded from Redis.")
    ...
    def save_results_to_json(self, organized_results: Dict[str, Any]) -> Optional[Path]:
        ...
        try:
            ...
            cache_json(f"results:{self.session_dir.name}", organized_results)
            return json_path
        except Exception as e:
...
async def startup_event():
    global pipeline
    try:
        # Check Redis server availability before anything else
        try:
            ...
            logger.critical(f"❌ Redis server is not available: {e}. Exiting.")
            sys.exit(1)
...
if __name__ == "__main__":
    uvicorn.run("comprehensive_pipeline_api_redis:app", host="0.0.0.0", port=8200, reload=True)